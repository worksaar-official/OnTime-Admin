...
class ItemController extends Controller
{
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        if (addon_published_status('TaxModule')) {
            $SystemTaxVat = \Modules\TaxModule\Entities\SystemTaxSetup::where('is_active', 1)->where('is_default', 1)->first();
            if ($SystemTaxVat?->tax_type == 'product_wise') {
                $activeTaxes = \Modules\TaxModule\Entities\Tax::where('is_active', 1)->get(['id', 'name', 'tax_rate']);
            }
        }

        $extractTaxIds = function ($collection) use ($activeTaxes) {
            $gst = data_get($collection, 'Gst');
            if ($gst === null) {
                $gst = data_get($collection, 'GST');
            }
            if ($gst === null) {
            ...
            if (is_string($gst)) {
                $gst = trim($gst);
                if ($gst === '' || in_array(strtolower($gst), ['0', 'no', 'none', 'n/a', 'na', 'null'], true)) {
                    return [];
                }
                if (Str::startsWith($gst, '[')) {
            ...
            $tokens = is_array($gst) ? $gst : preg_split('/[,\|;]+/', (string) $gst);
            $taxIds = [];
            ...
            foreach ($tokens as $token) {
                ...
                $tokenStr = is_string($token) ? trim($token) : trim((string) $token);
                $tokenStr = trim(str_replace('%', '', $tokenStr));
                ...
                if (is_numeric($tokenStr)) {
                    ...
                    if ($activeTaxes->firstWhere('id', $int)) {
                        $taxIds[] = $int;
                        continue;
                    }

                    $byRate = $activeTaxes->first(function ($t) use ($num) {
                        return abs(((float) $t->tax_rate) - $num) < 0.00001;
                    });
                    if ($byRate) {
                        $taxIds[] = (int) $byRate->id;
                    }
                    continue;
                }

                $lower = strtolower($tokenStr);
                $byName = $activeTaxes->first(function ($t) use ($lower) {
                    return strtolower((string) $t->name) === $lower;
                });
                if ($byName) {
                    $taxIds[] = (int) $byName->id;
                    continue;
                }

                if (preg_match('/(\d+(?:\.\d+)?)/', $tokenStr, $m)) {
                    $rate = (float) $m[1];
                    $byRate = $activeTaxes->first(function ($t) use ($rate) {
                        return abs(((float) $t->tax_rate) - $rate) < 0.00001;
                    });
                    if ($byRate) {
                        $taxIds[] = (int) $byRate->id;
                    }
            ...
            return array_values(array_unique(array_map('intval', $taxIds)));
        };
        ...
        if ($request->button == 'import') {
            ...
            try {
                foreach ($collections as $key => $collection) {
                    ...
                    $nextId = $item_id + $key + 1;
                    if ($SystemTaxVat?->tax_type == 'product_wise') {
                        $taxIds = $extractTaxIds($collection);
                        if ($taxIds) {
                            $taxableMap[$nextId] = $taxIds;
                        }
                    }
            ...
            try {
                ...
                foreach ($chunk_items as $key => $chunk_item) {
                    ...
                    foreach ($chunk_item as $item) {
                        ...
                        if ($SystemTaxVat?->tax_type == 'product_wise') {
                            foreach (($taxableMap[$insertedId] ?? []) as $tax_id) {
                                DB::table('taxables')->insert([
                                    'taxable_type' => Item::class,
                                    'taxable_id' => $insertedId,
                                    'system_tax_setup_id' => $SystemTaxVat->id,
                                    'tax_id' => $tax_id,
                                    'created_at' => now(),
                                    'updated_at' => now(),
                ...
                if (count($temp_data) > 0) {
                    ...
                    foreach ($chunk_temp_items as $key => $chunk_item) {
                        ...
                        if ($SystemTaxVat?->tax_type == 'product_wise') {
                            foreach (($taxableMap[$item['item_id']] ?? []) as $tax_id) {
                                DB::table('taxables')->insert([
        ...
        try {
            ...
            if (count($temp_data) > 0) {
                ...
                foreach ($chunk_items as $key => $chunk_item) {
                    ...
                    foreach ($chunk_item as $item) {
                        $tempId = $item['id'] ?? null;
                        if ($tempId && DB::table('temp_products')->where('id', $tempId)->exists()) {
                            DB::table('temp_products')->where('id', $tempId)->update($item);
                            Helpers::updateStorageTable(get_class(new TempProduct), $tempId, $item['image']);
                        } else {
                            $tempId = DB::table('temp_products')->insertGetId($item);
                            Helpers::updateStorageTable(get_class(new TempProduct), $tempId, $item['image']);
                        }

                        $srcItemId = $item['item_id'] ?? null;
                        if ($tempId && $srcItemId && $SystemTaxVat?->tax_type == 'product_wise' && isset($taxableMap[$srcItemId])) {
                            DB::table('taxables')->where('taxable_type', TempProduct::class)->where('taxable_id', $tempId)->delete();
                            foreach ($taxableMap[$srcItemId] as $tax_id) {
                                DB::table('taxables')->insert([
                                    'taxable_type' => TempProduct::class,
                                    'taxable_id' => $tempId,
                                    'system_tax_setup_id' => $SystemTaxVat->id,
                ...
                foreach ($chunk_items as $key => $chunk_item) {
                    ...
                    foreach ($chunk_item as $item) {
                        $itemId = $item['id'] ?? null;
                        if ($itemId && DB::table('items')->where('id', $itemId)->exists()) {
                            DB::table('items')->where('id', $itemId)->update($item);
                            Helpers::updateStorageTable(get_class(new Item), $itemId, $item['image']);
                        } else {
                            $itemId = DB::table('items')->insertGetId($item);
                            Helpers::updateStorageTable(get_class(new Item), $itemId, $item['image']);
                        }

                        if ($itemId && $SystemTaxVat?->tax_type == 'product_wise' && isset($taxableMap[$itemId])) {
                            DB::table('taxables')->where('taxable_type', Item::class)->where('taxable_id', $itemId)->delete();
                            foreach ($taxableMap[$itemId] as $tax_id) {
                                DB::table('taxables')->insert([
                                    'taxable_type' => Item::class,
                                    'taxable_id' => $itemId,
                                    'system_tax_setup_id' => $SystemTaxVat->id,
    ...
    public function bulk_export_data(Request $request)
    {
        ...
        $productWiseTax = $taxData['productWiseTax'] ?? false;
        // worksaar end
        $products = Item::when($request['type'] == 'date_wise', function ($query) use ($request) {
            ...
            // worksaar start
            ->when($productWiseTax, function ($query) {
                $query->with('taxVats');
            // worksaar end    
            })
