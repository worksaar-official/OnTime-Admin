...
class WalletController extends Controller
{
    ...
    public function w_request(Request $request)
    {
        ...
        $withdrawable_balance_after_tax = max(0, (float) $w->balance - (float) $gst_deduction_for_withdrawable);

        if ((string) $withdrawable_balance_after_tax >= (string) $request['amount'] && (string) $request['amount'] > .01) {
            $data = [
