...
class ItemController extends Controller
{
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        if ($request->button == 'import') {
            ...
            $taxableRowMap = [];
            try {
                foreach ($collections as $rowIndex => $collection) {
                    //worksaar end
...
//worksaar end
            ...
            try {
                ...
                foreach ($chunk_items as $key => $chunk_item) {
                    ...
                    //worksaar start
                    foreach ($chunk_item as $indexInChunk => $item) {
                    
                        $insertedId = DB::table('items')->insertGetId($item);
                        Helpers::updateStorageTable(get_class(new Item), $insertedId, $item['image']);

                        if ($SystemTaxVat?->tax_type == 'product_wise') {
                            $rowIndex = $key * $chunkSize + $indexInChunk;
                            foreach (($taxableRowMap[$rowIndex] ?? []) as $tax_id) {
    ...
    //worksaar start
                if ($SystemTaxVat?->tax_type == 'product_wise') {
                    $taxIds = $extractTaxIds($collection);
                    if ($taxIds) {
                        $taxableMap[(int) $collection['Id']] = $taxIds;
                    }
                }
    //worksaar end
                array_push($data, [
            ...
            foreach ($chunk_items as $key => $chunk_item) {
                ...
                foreach ($chunk_item as $item) {
                    ...
                    if ($itemId && DB::table('items')->where('id', $itemId)->exists()) {
                        ...
                        $itemId = (int) DB::table('items')->insertGetId($item);
                        Helpers::updateStorageTable(get_class(new Item), $itemId, $item['image']);
                    }
