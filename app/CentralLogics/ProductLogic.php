...
// worksaar start
            $gst = '';
            if ($productWiseTax) {
                $taxIds = $item->relationLoaded('taxVats')
                    ? $item->taxVats?->pluck('tax_id')->filter()->unique()->values()->all()
                    : $item->taxVats()->pluck('tax_id')->toArray();

                $gst = !empty($taxIds) ? implode(',', $taxIds) : '';
            }
// worksaar end
            $storage[] = [
                'Id'=>$item->id,
