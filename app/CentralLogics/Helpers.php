    ...
    public static function sendNotificationToHttp(array|null $data)
    {
        ...
        if (data_get($key, 'project_id')) {
            ...
            try {
                ...
                info("FCM Topic Notification Response: " . $response->body());
                if ($response->successful()) {
                    return true;
                }
            } catch (\Exception $exception) {
                info("FCM Topic Notification Exception: " . $exception->getMessage());
                return false;
            }
        } else {
            info("FCM Topic Notification FAILED: project_id missing in config");
        }
        return false;
    }
    ...
    public static function send_order_notification($order)
    {
        ...
        try {
            ...
            if (
                ...
                info("Sending admin notification for order: " . $order->id);
                self::send_push_notif_to_topic($data, 'admin_message', 'order_request', url('/') . '/admin/order/list/all');
            } else {
                info("Admin notification criteria NOT met for order: " . $order->id . " Status: " . $order->order_status . " Method: " . $order->payment_method);
            }
