    ...
    <table class="table table--vertical-middle">
        <thead class="text-muted thead-light">
            <tr class="text-center">
                ...
                <th class="border-bottom-0 border-top-0 text-right" scope="col">{{ translate('messages.Unit_price') }}
                </th>
                    ...
                    @if (is_array($cartItem))
                        ...
                        <tr>
                            <td class="media align-items-center cursor-pointer quick-View-Cart-Item"
                                ...
                                <img class="avatar avatar-sm mr-1 onerror-image" src="{{ $cartItem['image_full_url'] }}"
                                    data-onerror-image="{{ asset('public/assets/admin/img/100x100/2.png') }}"
                                    alt="{{ $cartItem['name'] }} image">
                                <div class="media-body">
                                    <h6 class="text-hover-primary mb-0 fs-12">{{ Str::limit($cartItem['name'], 14) }}
                                    </h6>
                            ...
                            <td class="text-center middle-align">
                                <input type="number" data-key="{{ $key }}"
                                    ...
                                    data-oldvalue="{{ $cartItem['quantity'] }}" value="{{ $cartItem['quantity'] }}"
                                    min="1"
                            ...
                            <td>
                                <div class="d-flex justify-content-center">
                                    <a href="javascript:" data-product-id="{{ $key }}"
                                        class="pos-cart-remove-btn remove-From-Cart rounded-circle"> <i
...
<div class="box p-3">
    <dl class="row text-dark">
        ...
        <dd class="col-6 text-right">- {{ \App\CentralLogics\Helpers::format_currency(round($discount_on_product, 2)) }}
        </dd>
    ...
    <form action="{{ route('admin.pos.order') }}?store_id={{ request('store_id') }}" id='order_place' method="post">
        ...
        <div class="row button--bottom-fixed g-1 bg-white">
            <div class="col-sm-6">
                <button type="button" class="btn h-100  btn-outline-danger btn-block empty-Cart"
                    {{ session()->has('cart') && count(session()->get('cart')) > 0 ? '' : 'disabled' }}>{{ translate('messages.Clear Cart') }}
                </button>
            </div>
            <div class="col-sm-6">
                <button type="submit"
                    class="btn  btn--primary place-order-submit btn-block">{{ translate('messages.place_order') }}
                </button>
            </div>
        </div>
    </form>
