                                        ...
                                        <form id="search-form" class="search-form">
                                            ...
                                            <div class="position-relative">
                                                ...
                                                <img width="16" height="16"
                                                    src="{{ asset('public/assets/admin/img/icons/search-icon.png') }}"
...
@push('script_2')
    <script
        src="https://maps.googleapis.com/maps/api/js?key={{ \App\Models\BusinessSetting::where('key', 'map_api_key')->first()->value }}&libraries=places,marker&callback=initMap&v=3.64">
    </script>
    ...
    <script>
        ...
        function initMap() {
            ...
            let map = new google.maps.Map(document.getElementById("map"), {
                zoom: 13,
                center: {
                    lat: {{ $store ? $store['latitude'] ?? '23.757989' : '23.757989' }},
                    lng: {{ $store ? $store['longitude'] ?? '90.360587' : '90.360587' }}
                },
                mapId: mapId
            });
                            ...
                            geocoder.geocode({
                                ...
                                if (status === google.maps.GeocoderStatus.OK) {
                                    if (results[1]) {
                                        ...
                                        const origin2 = "{{ addslashes($store->address) }}";
                                        ...
                                        service.getDistanceMatrix(request).then((response) => {
                                            ...
                                            function calcDeliveryFee(distancMileResult, extra_charge) {
                                                ...
                                                if (js_delivery_type === 'tier' && js_tiers.length > 0) {
                                                    if (js_accumulative === 1) {
                                                        ...
                                                        matched_temp_charge = matching_tier_rate; // Using rate as temp_charge indicator
                                            ...
                                            $.get({
                                                ...
                                                error: function() {
                                                    var delivery_charge = calcDeliveryFee(distancMileResult, 0);
                                                    document.getElementById('delivery_fee').value = delivery_charge;
                                                    jQuery('#delivery_fee').siblings('strong').html(delivery_charge + '{{ \App\CentralLogics\Helpers::currency_symbol() }}');
                                                }
                                            });
                ...
                ajax: {
                    ...
                    data: function(params) {
                        return {
                            ...
                            module_id: {{ Config::get('module.current_module_id') ?? 0 }},
                            page: params.page
                        };
                    },
        ...
        function checkStore() {
            let module_id = {{ Config::get('module.current_module_id') ?? 0 }};
        ...
        function updateCart() {
            ...
            $.post('<?php echo e(route('admin.pos.single_items')); ?>' + window.location.search, {
                _token: '<?php echo e(csrf_token()); ?>'
            }, function(data) {
