...
<div class="row g-3">
    ...
    // worksaar start
    $gst_deduction_for_withdrawable = \App\Models\OrderTransaction::NotRefunded()
        ->where('vendor_id', $wallet->vendor_id)
        ->sum('tax');

    $withdrawable_balance_after_tax = max(0, (float)$wallet->balance - (float)$gst_deduction_for_withdrawable);
    $wallet_earning_after_tax = max(0, (float)$wallet_earning - (float)$gst_deduction_for_withdrawable);
    $total_earning_after_tax = max(0, (float)$wallet->total_earning - (float)$gst_deduction_for_withdrawable);
      //worksaar end  
    ...
    <div class="col-md-12">
        <div class="row g-3">
            ...
            <div class="col-sm-{{ isset($col_size)  == true ? '3' :'4' }}">
                <div class="resturant-card shadow--card-2">
                    <h4 class="title">{{\App\CentralLogics\Helpers::format_currency($withdrawable_balance_after_tax)}}</h4>
                ...
                <div class="resturant-card shadow--card-2">
                    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                        <div>
                            ...
                            @if ($wallet->balance > 0)
                                <h4 class="title">{{\App\CentralLogics\Helpers::format_currency($wallet_earning_after_tax)}}</h4>
        ...
        <div class="row g-3">
            ...
            <div class="col-sm-4">
                <div class="resturant-card  bg--1">
                    <h4 class="title">{{\App\CentralLogics\Helpers::format_currency($total_earning_after_tax)}}</h4>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="amount" step="0.01"
                               value="{{ number_format($withdrawable_balance_after_tax, 2, '.', '') }}"
                               class="form-control h--45px" id="" min="1" max="{{ number_format($withdrawable_balance_after_tax, 2, '.', '') }}">
                               <!-- worksaar end -->
                    </div>
                </div>
